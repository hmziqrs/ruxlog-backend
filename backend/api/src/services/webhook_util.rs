//! Shared primitives for webhook signature verification (always-on).
//!
//! Every provider that receives inbound webhooks (billing providers, the mail
//! bounce/complaint receiver) authenticates the raw body here. Previously these
//! primitives lived under `services::billing::webhook_util`, which made them
//! unreachable from any feature that is not `billing` — so the mail webhook
//! receiver would not compile under `--features mail-cloudflare` alone. They
//! were promoted to this always-on module; `billing::webhook_util` now
//! re-exports them so existing billing call sites are unchanged.
